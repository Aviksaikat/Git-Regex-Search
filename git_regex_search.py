...
def process_env():
    ...
    if access_token is None:
        try:
            ...
            print(
                "Error: Could not find access token in environment variable or config.toml file."
            )
            exit(1)
...
def Search(url, pattern):
    ...
    if url:
        if url.endswith(".txt"):
                ...
                for url in urls:
                    try:
                        ...
                        for result in results:
                            print(
                                f"{url.strip()}/tree/master/{result[0].path}#L{result[1]}"
                            )
            ...
            try:
                ...
                for result in results:
                    print(
                        colored(
                            f"{url}/tree/master/{result[0].path}#L{result[1]}",
                            "magenta",
                        )
                    )
            except Exception as e:
        ...
        try:
            ...
            for result in results:
                print(
                    colored(f"{url}/tree/master/{result[0].path}#L{result[1]}", "green")
                )
        except Exception as e:
            print(f"Error: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--url", help="URL of the repository (single or file containing URLs)"
    )
